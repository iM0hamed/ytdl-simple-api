function inicio() {
  ...
  if (!linkvideo || linkvideo.length < 20)
    return showAlert("Fill in the url correctly", "alert");
  ...
  fetch(`/info?url=${linkvideo}`, requestOptions)
    .then((response) => {
      if (response.status != 200) {
        showAlert("error: the server did not respond as it should :(", "alert");
          ...
          .then((data) => {
            if (!data.sucess) {
              if (data.error.includes("410")) {
                return showAlert(
                  "Error: sorry, this video has viewing restrictions, I cant download :(",
                  "alert"
                );
              }
              return showAlert("erro: " + data.error, "alert");
            }
    ...
    .then((response) => {
      if (response.status != 200) {
        ...
        response
          .json()
          .then((data) => {
            if (!data.sucess) return showAlert(("erro: ", data.error), "alert");
...
function resetBtnBuscar() {
  ...
  document.getElementById(
    "botaobuscar"
  ).innerHTML = `<p type="button" class="btn btn-lg btn-danger mt-3" id="botaobuscar" onclick="inicio()">Search <span class="material-icons">search</span></p>`;
}
  ...
  return `
    <p class="h3 text-muted">${data.title}</p>
        ...
        <p class="mt-3 text-light text-muted">
            <strong>duration:</strong> ${sToTime(
              data.duration
        ...
        <div id="divDownload" class="mt-4">
            ...
            <p type="button" class="btn btn-lg btn-danger" id="botaovideo" onclick="download('video_${
              data.videoid
            }')">Video <span class="material-icons">smart_display</span></p>
            <p type="button" class="btn btn-lg btn-danger" id="botaoaudio" onclick="download('audio_${
              data.videoid
            }')">Audio <span class="material-icons">audiotrack</span></p>
        </div>

        <p type="button" class="btn btn-lg mt-2" id="botaovoltar" onClick="window.location.reload();">Return</p>`;
}
