...
app.listen(porta, function () {
  ...
  if (porta == 3000) {
    console.log("running locally http://localhost:3000");
  }
});
app.get("/url", async function (req, res) {
  res.send("website base url: " + (await myhost(req)));
});
...
app.get("/audio", async function (req, res) {
  ...
  if (!urlvideo || urlvideo.length < 11)
    return res.json({ sucess: false, error: "no url or invalid url" });

  try {
    const video1 = ytdl(urlvideo, {
      requestOptions: { headers: { cookie: COOKIE } },
    });

    videoinfo = await getInfo(urlvideo);
    var nomearquivo = videoinfo.videoid
      ? "audio_" + videoinfo.videoid
      : "audio_" + getRandom("");
    ...
    ffmpeg(video1)
      ...
      .on("end", () => {
        myhost(req).then((url) => {
          res.json({
            sucess: true,
            file: `${url}/arquivo/?arquivo=${nomearquivo}.mp3`,
          });
        });
      })
  ...
  try {
    ...
    video2.on("end", () => {
      myhost(req).then((url) => {
        res.json({
          sucess: true,
          file: `${url}/arquivo/?arquivo=${nomearquivo}.mp4`,
        });
      });
    });

    video2.pipe(
      fs.createWriteStream(`${__dirname}/publico/${nomearquivo}.mp4`)
    );
  } catch (e) {
    console.log("erro ", e);
    res.json({ sucess: false, error: e.message });
...
app.get("/arquivo", function (req, res) {
  ...
  if (!nomearquivo || !fs.existsSync(caminho))
    return res.json({ sucess: false, error: "no url" });
...
app.get("/info", async function (req, res) {
  ...
  if (!link || link.length < 11) res.json({ sucess: false, error: "no url" });
...
async function getInfo(url) {
  try {
    return await ytdl
      .getInfo(url, { requestOptions: { headers: { cookie: COOKIE } } })
      .then((info) => {
        return {
          sucess: true,
